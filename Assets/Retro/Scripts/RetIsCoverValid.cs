    ...
    public class RetIsCoverValid : Condition
    {
        ...
        public override bool Check(Packet packet)
        {
            if (cover.IsValid(packet) && self.IsValid(packet))
            {
                ...
                var distance = Vector3.Distance(identity.Transform.position.Flatten(), self.Value.position.Flatten());
                if (distance > maxDistance) return false;
                
                var height = cover.Value.transform.position.y;
                var direction = identity.Transform.position.Flatten(height) - cover.Value.transform.position;
